...
class MySDSPipeline(StableDiffusionPipeline):
    r"""
    ...
    def schedule_timestep(self, step, min_step = None):
    ...
    def sds_default(self, 
        ...
        latent_list = [self.encode_(pred_rgb_[i].unsqueeze(0)) for i in range(batch_size)]
        ...
        if noise_step is None: 
            noise_step = self.schedule_timestep(step)
        ...
        noise_pred = self.predict_noise(latents= latents, 
                            noise = noise,
                            time_step= noise_step, 
                            embeddings= text_embeddings, 
                            guidance_scale= guidance_scale_) # vectorfusion은 guidance scale이 100 
        
        w = (1 - alphas[noise_step])
        grad = grad_scale * w * (noise_pred - noise)
        grad = torch.nan_to_num(grad)
        loss = SpecifyGradient.apply(latents, grad)

        return loss, grad.mean(), noise_step
